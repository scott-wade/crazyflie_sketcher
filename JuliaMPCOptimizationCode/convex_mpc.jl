...
function convex_mpc(A::Matrix, # discrete dynamics matrix A
    ...
    for k = 1:(N_mpc-1)
        ...
        prob.constraints = vcat(prob.constraints, X[:, k+1] == A*xk + B*uk)

        # control bound constraints
        prob.constraints = vcat(prob.constraints, uk <= u_max)
        prob.constraints = vcat(prob.constraints, uk >= u_min)
        
        # state bounds
        prob.constraints = vcat(prob.constraints, xk <= x_max)
        prob.constraints = vcat(prob.constraints, xk >= x_min)
    end
