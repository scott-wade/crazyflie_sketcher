...
function euler_to_quaternion(roll::Float64, pitch::Float64, yaw::Float64)
...
function quad_dynamics(model, x, u)
    ...
    r = x[1:3]
    q = x[4:7] / LinearAlgebra.norm(x[4:7])  # Normalize quaternion
    v = x[8:10]
    ω = x[11:13]
    Q = qtoQ(q)
    ...
    mass = model.mass
    g = model.g
    μ = 0.08  # Coefficient of kinetic friction
    
    # Friction force
    f_friction = μ * mass * g * sign.(v) .* max.(abs.(v), 1e-6)
    # Magnet force
    magnet_force = [0; 0; -2.00]  # Approx. 0.9 lb force converted to N
    ...
    v̇ = [0; 0; -g] + (1/mass) * (Q * [zeros(2, 4); kt * ones(1, 4)] * u + magnet_force - f_friction)
