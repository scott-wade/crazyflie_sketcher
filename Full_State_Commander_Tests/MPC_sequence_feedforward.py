...
def parse_MPC_output(filename):
    ...
    with open(filename, 'r') as file:
        ...
        for row in csv_reader:
            row = row[0:14]
            row2 = [float(element) for element in row[:13]]  # Parse first 13 values as float
            thrust = float(row[13])  # Parse the 14th value as thrust
            Xref.append(row2)
            ffThrusts.append(thrust)
    ...
    Xref = np.array(Xref)  # Convert Xref to numpy array
    accels = np.array(accels)  # Convert accels to numpy array
    ffThrusts = np.array(ffThrusts).reshape(-1, 1)  # Reshape thrusts to a column vector
    output = np.hstack((Xref, accels, ffThrusts))  # Horizontally stack all arrays
    return output
...
def traj2ref(Xref):
    ...
    return pos, vel, acc, ori, rollrate, pitchrate, yawrate, thrust
...
def run_MPC_sequence(scf, log_conf):
    ...
    pos, vel, acc, ori, rollrate, pitchrate, yawrate, thrust = traj2ref(csv_output[0])
    ...
    for full_state in csv_output:
        pos, vel, acc, ori, rollrate, pitchrate, yawrate, thrust = traj2ref(full_state)

        # Update base thrust with ff term
        cf.param.set_value('posCtlPid.thrustBase', thrust)
